...
@pytest.mark.skipif(not _HAS_CUDA, reason="Triton Predictor kernel needs CUDA")
def test_gather_codec_embedding_and_add_stores_tokens_in_cuda_graph():
    device = torch.device("cuda")
    token_ids = torch.tensor([1, 3], dtype=torch.long, device=device)
    embedding_weight = torch.randn(8, 8, dtype=torch.bfloat16, device=device)
    initial_accumulated = torch.randn(2, 8, dtype=torch.bfloat16, device=device)
    expected_gathered = F.embedding(token_ids, embedding_weight)
    expected_accumulated = initial_accumulated.clone()
    expected_accumulated.add_(expected_gathered)
    gathered = torch.empty_like(expected_gathered)
    accumulated = initial_accumulated.clone()
    output_token_storage = torch.full(
        (2, 2), -1, dtype=torch.long, device=device
    )
    output_token_ids = output_token_storage[:, 1]

    # Compile the Triton specialization before graph capture.
    assert gather_codec_embedding_and_add(
        token_ids,
        embedding_weight,
        gathered,
    ...
    torch.cuda.synchronize()
    accumulated.copy_(initial_accumulated)
    output_token_storage.fill_(-1)
